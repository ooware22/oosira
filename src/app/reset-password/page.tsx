...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setStatus('error');
      setErrorMessage('Missing reset token. Please check your email link.');
      return;
    }
  ...
  return (
    <>
      <h1 className="text-2xl font-bold text-txt text-center mb-2">Reset Password</h1>
      <p className="text-txt-muted text-center text-sm mb-8">Choose a new password for your account.</p>
      ...
      {status === 'success' ? (
        ...
        >
          ...
          <h2 className="text-xl font-bold text-txt mb-2">Password Reset!</h2>
          <p className="text-sm text-txt-muted text-center mb-6">Your password has been changed successfully. Redirecting you to login...</p>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="space-y-1.5">
            <label className="block text-[11px] font-bold text-txt-muted uppercase tracking-wider">New Password</label>
            <input
              type="password"
              required
              value={password}
          ...
          <div className="space-y-1.5">
            <label className="block text-[11px] font-bold text-txt-muted uppercase tracking-wider">Confirm New Password</label>
            <input
              type="password"
              required
      ...
      <main className="flex-1 flex items-center justify-center px-4 relative z-10 py-20">
        ...
        >
          ...
          <div className="bg-surface/80 backdrop-blur-2xl border border-border rounded-3xl p-8 sm:p-10 shadow-2xl shadow-black/5">
            <Suspense fallback={<div className="text-center p-8 text-txt-muted text-sm border border-border/50 rounded-xl bg-surface2/50">Loading form...</div>}>
              <ResetPasswordForm />
            </Suspense>
          </div>
        </motion.div>
