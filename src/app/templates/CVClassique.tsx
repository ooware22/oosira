...
/**
 * Describe the CV as measurable blocks. The paginator distributes these across
 * real page elements; the component below renders them as one continuous flow.
 */
export function buildClassiqueLayout(
  data: Candidate,
            ...
            block(
              ...
              <div className="cv-exp-item">
                <div className="exp-header">
                  ...
                  <span className="exp-date" data-cv-field={`experiences.${i}.dateDebut`}>{dateRangeLabel(exp, language)}</span>
                </div>
...
export function CVClassique({ data, config }: { data: Candidate, config?: CVStyleConfig }) {
  ...
  const layout = buildClassiqueLayout(data, config, t, language);
