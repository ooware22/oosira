          ...
          <div key="competences" className="exec-sidebar-section">
            ...
            <div className="exec-tag-list">
              {data.competences.map((s, i) => (
                <span className="exec-tag" key={i}>{s}</span>
              ))}
            </div>
          </div>
        );
          ...
          <div key="langues" className="exec-sidebar-section">
            ...
            {data.langues.map((l, i) => (
              <div className="exec-lang-row" key={i}>
                <div className="exec-lang-name">{l.langue}</div>
          ...
          <div key="logiciels" className="exec-sidebar-section">
            ...
            <div className="exec-tag-list">
              {data.logiciels.map((s, i) => (
                <span className="exec-tag exec-tag-soft" key={i}>{s}</span>
              ))}
            </div>
          </div>
        );
        ...
        return (
          <div key="experiences" className="exec-sidebar-section">
            ...
            {data.experiences.map((exp, i) => (
              <div className="exec-lang-row" key={i}>
                <div className="exec-lang-name">{exp.poste}</div>
                <div className="exec-lang-detail">{exp.entreprise}, {fmtDate(exp.dateDebut)} - {fmtDate(exp.dateFin)}</div>
              </div>
            ))}
          </div>
        );
        ...
        return (
          <div key="formations" className="exec-sidebar-section">
            ...
            {data.formations.map((f, i) => (
              <div className="exec-lang-row" key={i}>
                <div className="exec-lang-name">{f.diplome}</div>
                <div className="exec-lang-detail">{f.etablissement}, {f.dateDebut && f.dateFin ? `${fmtDate(f.dateDebut)} - ${fmtDate(f.dateFin)}` : f.annee}</div>
              </div>
            ))}
          </div>
        );
            ...
            {data.experiences.map((exp, i) => (
              <div className="exec-entry" key={i}>
                <div className="exec-entry-header">
                  <div>
                    <div className="exec-entry-role">{exp.poste}</div>
                    <div className="exec-entry-company">{exp.entreprise}, {exp.secteur}</div>
                  </div>
                  <div className="exec-entry-date">{fmtDate(exp.dateDebut)} - {fmtDate(exp.dateFin)}</div>
                </div>
                <p className="exec-entry-desc">{exp.description}</p>
          ...
          <section className="exec-section" key="formations">
            ...
            {data.formations.map((f, i) => (
              <div className="exec-entry" key={i}>
                <div className="exec-entry-header">
                  <div>
                    <div className="exec-entry-role">{f.diplome}, {f.specialite}</div>
                    <div className="exec-entry-company">{f.etablissement}, {f.ville}</div>
                  </div>
                  <div className="exec-entry-date">{f.dateDebut && f.dateFin ? `${fmtDate(f.dateDebut)} - ${fmtDate(f.dateFin)}` : f.annee}</div>
                </div>
                {f.mention && <span className="exec-mention">{f.mention}</span>}
        ...
        return (
          <section className="exec-section" key="competences">
            ...
            <div className="exec-tag-list" style={{ gap: 5 }}>
              {data.competences.map((s, i) => (
                <span className="exec-tag" key={i} style={{ background: 'var(--cv-accent-10, rgba(59,130,246,0.1))', color: 'var(--cv-accent, #3b82f6)', border: '1px solid var(--cv-accent, #3b82f6)', borderColor: 'rgba(59,130,246,0.2)' }}>{s}</span>
              ))}
            </div>
          </section>
        );
          ...
          <section className="exec-section" key="langues">
            ...
            {data.langues.map((l, i) => (
              <div className="exec-entry" key={i} style={{ marginBottom: 6, paddingBottom: 0, borderBottom: 'none' }}>
                <div className="exec-entry-header">
                  <div className="exec-entry-role">{l.langue}</div>
      ...
      case 'logiciels':
        ...
        return (
          <section className="exec-section" key="logiciels">
            ...
            <div className="exec-tag-list" style={{ gap: 5 }}>
              {data.logiciels.map((s, i) => (
                <span className="exec-tag" key={i} style={{ background: 'rgba(59,130,246,0.04)', color: 'var(--cv-accent, #3b82f6)' }}>{s}</span>
              ))}
            </div>
          </section>
        );
    ...
    <div className="cv-page cv-executif">
      ...
      <main className="exec-main">
        ...
        <div className="exec-header">
          <h1 className="exec-name">{data.prenom} {data.nom}</h1>
          <div className="exec-title">{data.titre}</div>
          {data.accroche && <p className="exec-accroche">{data.accroche}</p>}
        </div>
